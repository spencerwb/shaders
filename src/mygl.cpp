...
void MyGL::render3DScene()
{
    ...
    // set the surface and post-process shaders's time attributes
...
void MyGL::createShaders()
{
    ...
    std::shared_ptr<SurfaceShader> soul = std::make_shared<SurfaceShader>(this);
    soul->create(":/glsl/surface/soul.vert.glsl", ":/glsl/surface/soul.frag.glsl");
    m_surfaceShaders.push_back(soul);
    ...
    std::shared_ptr<PostProcessShader> thematrix = std::make_shared<PostProcessShader>(this);
    thematrix->create(":/glsl/post/passthrough.vert.glsl", ":/glsl/post/thematrix.frag.glsl");
    m_postprocessShaders.push_back(thematrix);
