...
void Mesh::createCube(const char *textureFile, const char *bgTextureFile)
{
    ...
    // VBO data for positions, normals, UVs, and indices of a cube that spans
    // the range [-1, 1] in the X, Y, and Z axes in world space.
    // For each face, if you are looking at the face head-on, the vertex data on
    // the quad are listed in counter-clockwise order starting at the bottom-left
    // vertex (BL -> BR -> TR -> TL).
